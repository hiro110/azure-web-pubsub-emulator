...
Download the latest release binary from the [Releases](../../releases) page, then:

```bash
./web-pubsub-emulator --config config.yaml
```

Once started, the emulator prints the connection string:
...
All endpoints require HMAC-SHA256 authentication (`Authorization: HMAC-SHA256 ...`).
