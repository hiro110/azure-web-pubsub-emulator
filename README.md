...
**最小構成 — AccessKey を環境変数で渡すだけで起動できます：**
...
付属の `docker-compose.yml` はローカル開発用のデフォルト設定です。AccessKey や port をオーバーライドするには環境変数をそのまま設定します：
...
[Releases](https://github.com/hiro110/azure-web-pubsub-emulator/releases) ページから OS / アーキテクチャに合ったバイナリをダウンロードして実行します：

```bash
./web-pubsub-emulator --config config.yaml
# または AccessKey だけ指定（config ファイル不要）
...
All endpoints require authentication. 以下の2方式に対応しています：

| 方式 | ヘッダー | 使用する SDK |
|------|---------|------------|
| HMAC-SHA256 | `Authorization: HMAC-SHA256 SignedHeaders=...&Signature=...` | 旧バージョン / curl |
